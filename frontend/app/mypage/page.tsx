...
export default function MyPage() {
  ...
  const fetchWatchHistory = () => {
    const token = localStorage.getItem("token");
    if (!token) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/watch-history`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => (res.ok ? res.json() : []))
      .then((data) => setWatchHistory(Array.isArray(data) ? data : []))
  ...
  return (
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      {loading ? (
        ...
        >
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "72px 1fr 110px 120px 100px",
              padding: "0.6rem 1rem",
              background: "#f9f9f9",
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderBottom:
                    i < videos.length - 1 ? "1px solid #f0f0f0" : "none",
      ...
      <div style={{ marginTop: "3rem" }}>
        <h2 style={{ fontSize: "1.2rem", marginBottom: "1rem" }}>視聴履歴</h2>
        {watchHistory.length === 0 ? (
          <p style={{ color: "#aaa" }}>視聴履歴はありません</p>
        ) : (
